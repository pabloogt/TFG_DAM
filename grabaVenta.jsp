...
<%
    ...
    String idClienteParam = request.getParameter("ID_Ventas");

    if (idClienteParam != null && !idClienteParam.isEmpty()) {
        try {
            int idCliente = Integer.parseInt(idClienteParam);

            String consultaNumSocio = "SELECT * FROM Cabecera_Ventas WHERE ID_Ventas = " + idCliente;
            ...
            if (numeroDeSocios.next()) {
            	...
            	String insercion = "INSERT INTO Cabecera_Ventas VALUES ("
                        + idCliente + ", "
                        + Integer.valueOf(request.getParameter("N_Factura")) + ", "
                        + Integer.valueOf(request.getParameter("Codigo_clientes")) + ", '"
                        + request.getParameter("Nombre_clientes") + "', '"
    			...
    			s.execute(insercion);
                out.println("Cabecera_venta dado de alta correctamente");
            }
        } catch (NumberFormatException e) {
            out.println("Error al convertir ID_Ventas a entero: " + e.getMessage()+"");
        }
    } else {
        out.println("El parámetro ID_Ventas es nulo o vacío");
    }
    idClienteParam = request.getParameter("ID_Ventas");

    if (idClienteParam != null && !idClienteParam.isEmpty()) {
        try {
            int idCliente = Integer.parseInt(idClienteParam);

            String consultaNumSocio = "SELECT * FROM Detalle_Ventas WHERE N_Linea = " + idCliente;
            ResultSet numeroDeSocios = s.executeQuery(consultaNumSocio);

            if (numeroDeSocios.next()) {
                out.println("Lo siento, no se ha podido dar de alta, ya existe una compra con el mismo id (" + idCliente + ").");
            } else {
            	String insercion = "INSERT INTO Detalle_Ventas VALUES ("
                        + idCliente + ", "
    			...
    			s.execute(insercion);
                out.println("Detalle_Ventas dado de alta correctamente");
            }
        } catch (NumberFormatException e) {
...
<br>
<a href="indexV.jsp" class="btn btn-primary"><span class="glyphicon glyphicon-home"></span> Página principal</a>
